...
const LeaveRecord = (props) => {
  ...
  const isUserManager = leave?.userMangagerId === userAuth?._id;
  console.log("leave?.user?.userMangagerId", isUserManager);
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="bl-apply-leave_footer">
            ...
            {!leave?.isApproved &&
              !leave?.isRejected &&
              (isUserManager || userAuth?.isAdmin) && (
                <>
