...
const OrgAddProfile = () => {
  ...
  const { isProfileAdded, appErr, serverErr, managersList } = users;
  const { _id } = users?.userAuth;
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      managerDataId: managersList?.[0]?.id ? managersList?.[0]?.id : null,
      ProjectTeam: TeamList?.[0]?.TeamName,
      basicInformation: {
        firstName: "",
        lastName: "",
        employerId: "BSS",
    ...
    onSubmit: (values) => {
      ...
      console.log(values, "bloodGroup values");
    },
  });

  if (
    isProfileAdded ||
    (!proAdminAccessGivenFun(users?.userAuth?.Access) &&
      users?.userAuth?.Access)
  )
              ...
              <div>
                ...
                <div className="cs_edit_left_right_div">
                  <div className="cs_edit_left_input_right_input">
                    ...
                    <div className="cs_edit_input_div">
                      <h1 className="cs_edit_left_input">Password:</h1>
                      <input
