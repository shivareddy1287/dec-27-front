...
const OrgUpdateExitDetails = () => {
  ...
  const { _id, Access } = userDet?.userAuth;
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(updateexitDetailsAction({ id, values }));
      console.log(values, "values");
    },
  });

  if (isexitDetailseUpdated || (!normalAdminAccessGivenFun(Access) && Access))
          ...
          <form onSubmit={formik.handleSubmit} className="cs_edit_div">
            <div>
              ...
              >
                ...
                <div>
                  <h2 className="cs_edit_employee_head"> Edit Separation</h2>
                </div>
              </Link>
            </div>
...
export default OrgUpdateExitDetails;
